...
packets_filename = os.path.join(dirname, 'data/filtered_packets_5000_24.pickle')
...
def main():
    packets = []
    pattern = os.path.join('./data/', '*.pcap*')
...
#         packets.append(make_packets(file_name)) 
                       
#     print(packets)

    with open(packets_filename, 'wb') as file:
#         file.write(pickle.dumps(packets))
        file.write(pickle.dumps(filtered_packets))
